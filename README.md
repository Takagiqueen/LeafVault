...
然后打开 `.env`，本地非 Docker 开发建议把下面两项改为项目内相对路径：
...
本地测试阶段 `SECRET_KEY` 可以先保留示例值；生产环境必须改成长随机字符串。
...
如果要按生产部署方式测试，请使用 `.env.production.example` 和生产 Compose 文件。
...
正式部署由 Caddy 暴露 80/443，LeafVault 容器只在 Docker 网络内 expose 8000，不建议直接暴露应用容器端口。
