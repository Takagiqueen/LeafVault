...
本文用于说明 LeafVault v0.1 公网部署前后的安全边界、当前状态、生产推荐配置和后续收口路线。它面向个人自托管和小范围公开体验
...
- LeafVault v0.1 不是企业级多租户系统。
- SQLite 适合个人和少量朋友自托管，不适合高并发公开服务。
- 当前仍保留部分历史兼容路径，例如 Bearer fallback、少量 inline 样式/脚本兼容和 Cookie-only 迁移期。
- 云端密文快照和密文增量不等于所有服务器数据都完全零知识；公开介绍时应使用“云端同步工件以密文保存”这类更准确的描述。
